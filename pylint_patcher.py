...
class CodeFixer:
    ...
    def fix_trailing_whitespace(self, issue: PylintIssue) -> bool:
        ...
        if "Trailing whitespace" not in issue.message and "C0303" not in issue.code:
            return False
    ...
    def fix_f_string_without_interpolation(self, issue: PylintIssue) -> bool:
        """Behebt f-Strings ohne Interpolation (W1302, W1203)."""
        if "W1203" not in issue.code and "Consider using % formatting" not in issue.message:
            return False
            
        lines = self.load_file(issue.file_path)
        line = lines[issue.line_num - 1]
        
        # Suche nach Mustern wie logging.info(f"...")
        patterns = [
            (r'logging\.debug\(f(["\'])(.*?)\1', r'logging.debug(\1\2\1'),
            (r'logging\.info\(f(["\'])(.*?)\1', r'logging.info(\1\2\1'),
            (r'logging\.warning\(f(["\'])(.*?)\1', r'logging.warning(\1\2\1'),
            (r'logging\.error\(f(["\'])(.*?)\1', r'logging.error(\1\2\1'),
            (r'logging\.critical\(f(["\'])(.*?)\1', r'logging.critical(\1\2\1'),
            (r'logger\.debug\(f(["\'])(.*?)\1', r'logger.debug(\1\2\1'),
            (r'logger\.info\(f(["\'])(.*?)\1', r'logger.info(\1\2\1'),
            (r'logger\.warning\(f(["\'])(.*?)\1', r'logger.warning(\1\2\1'),
            (r'logger\.error\(f(["\'])(.*?)\1', r'logger.error(\1\2\1'),
            (r'logger\.critical\(f(["\'])(.*?)\1', r'logger.critical(\1\2\1')
        ]
        
        # Nur ersetzen, wenn keine Interpolation im String vorhanden ist
    ...
    def fix_except_without_exception(self, issue: PylintIssue) -> bool:
        """Behebt allgemeine except-Anweisungen (W0718)."""
        if "W0718" not in issue.code and "Catching too general exception" not in issue.message:
            return False
            
        lines = self.load_file(issue.file_path)
        line = lines[issue.line_num - 1]
        
        # Suche nach "except:" und ersetze durch "except Exception:"
    ...
    def fix_import_outside_toplevel(self, issue: PylintIssue) -> bool:
        """Markiert Importe außerhalb des Toplevel (C0415)."""
        if "C0415" not in issue.code and "Import outside toplevel" not in issue.message:
            return False
            
        lines = self.load_file(issue.file_path)
        line = lines[issue.line_num - 1]
        
        # Füge einen Kommentar hinzu, um zu erklären, warum der Import hier ist
        if re.search(r'^\s*import\s+\w+', line) or re.search(r'^\s*from\s+[\w.]+\s+import', line):
            indent = len(line) - len(line.lstrip())
            comment = ' ' * indent + '# pylint: disable=C0415  # Import außerhalb des Toplevel\n'
            lines.insert(issue.line_num - 1, comment)
            return True
            
        return False
        
    def fix_encoding_specified(self, issue: PylintIssue) -> bool:
        """Behebt fehlende Kodierungsangabe (W1514)."""
        if "W1514" not in issue.code and "Using open without explicitly specifying an encoding" not in issue.message:
            return False
            
        lines = self.load_file(issue.file_path)
        line = lines[issue.line_num - 1]
        
        # Suche nach open() ohne encoding-Parameter
        matches = re.findall(r'open\(\s*([^,]+)\s*,\s*([^,)]+)\s*\)', line)
        if matches:
            for match in matches:
                # Überprüfe, ob encoding fehlt
                if 'encoding' not in line:
                    old_open = f"open({match[0]}, {match[1]})"
                    new_open = f"open({match[0]}, {match[1]}, encoding='utf-8')"
                    lines[issue.line_num - 1] = line.replace(old_open, new_open)
                    return True
                    
        return False
    
    def fix_missing_docstring(self, issue: PylintIssue) -> bool:
        """Fügt fehlende Docstrings hinzu (C0111, C0112, C0114, C0115, C0116)."""
        if not any(code in issue.code for code in ["C0111", "C0112", "C0114", "C0115", "C0116"]) and "docstring" not in issue.message.lower():
            return False
    ...
    def fix_issues(self) -> int:
        ...
        for issue in self.issues:
            ...
            elif issue.code in ["C0111", "C0112", "C0114", "C0115", "C0116"]:  # Missing docstring
                if "docstring" in issue.message.lower():
                    fixed = self.fix_missing_docstring(issue)
            elif issue.code == "C0103":  # Bad variable name
                if "name" in issue.message.lower():
                    fixed = self.fix_bad_variable_name(issue)
            elif issue.code == "C0301":  # Line too long
                fixed = self.fix_line_too_long(issue)
            elif issue.code == "W1203":  # Consider using % formatting
                fixed = self.fix_f_string_without_interpolation(issue)
            elif issue.code == "W0718":  # Catching too general exception
                fixed = self.fix_except_without_exception(issue)
            elif issue.code == "C0415":  # Import outside toplevel
                fixed = self.fix_import_outside_toplevel(issue)
            elif issue.code == "W1514":  # Using open without encoding
                fixed = self.fix_encoding_specified(issue)
...
def main():
    ...
    parser.add_argument('--patch-file', help='Pfad zur Patch-Datei (fixes.patch)', 
                      default='fixes.patch')
    ...
    # Überprüfe, ob der angegebene Patch-Dateiname ein Verzeichnis ist
    if args.create_patch and os.path.isdir(args.patch_file):
        print(f"Fehler: '{args.patch_file}' ist ein Verzeichnis. Bitte geben Sie einen Dateinamen an.")
        sys.exit(1)
    ...
    if args.create_patch and not args.dry_run and fixes > 0:
        create_patch_file(fixer.file_cache, args.patch_file)
        print(f"Patch-Datei erstellt: {args.patch_file}")
