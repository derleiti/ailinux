...
class DuplicateCodeFinder:
    ...
    def analyze_duplicates(self) -> None:
        ...
        self.logger.info(f"Found {len(self.duplicate_sections)} duplicate code sections.")
        ...
        for files_key, sections in duplicates_by_files.items():
            ...
            self.logger.info("\n" + "="*80)
            self.logger.info(f"Duplicate code across {len(files)} files:")
            for file in files:
            ...
            self.logger.info(f"  1. Create a new module: {module_path}")
            self.logger.info(f"  2. Extract the following code to the new module:")
            
            # Display the duplicate code
            for i, section in enumerate(sections):
                self.logger.info(f"\n  --- Duplicate section {i+1} ---")
            ...
            for file in files:
                ...
                if not rel_path.startswith("."):
                    rel_path = f".{rel_path}"
                self.logger.info(f"  In {file}: from {rel_path} import *")
    ...
    def generate_refactoring_code(self) -> Optional[str]:
        ...
        module_content += "This module contains functionality that was previously duplicated across:\n"
        for file in files:
